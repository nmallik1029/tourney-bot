...
async def handle_krunker_webhook(request: web.Request) -> web.Response:
    ...
    if any(discord.utils.get(g.text_channels, name="test-raw-data") for g in bot.guilds):
        asyncio.create_task(_dump_raw_data())

    event_type = payload.get("type")
    if event_type != "match_end":
        return web.Response(status=200, text="ok")

    # PUG matches are handled by their own module; tournament logic falls through.
    try:
        from pug.match import try_handle_pug_match_end
        if await try_handle_pug_match_end(payload):
            return web.Response(status=200, text="pug-ok")  # matched a PUG and posted result
    except Exception as e:
        print(f"[Pug] webhook handler error: {e}")
